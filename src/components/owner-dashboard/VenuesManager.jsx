...
export default function VenuesManager({ venues }) {
  ...
  const handleSave = async (venueId, courtId) => {
    ...
    try {
      ...
      setLocalVenues((prev) =>
        prev.map((v) => v.id === venueId ? { ...v, courts: v.courts.map((c) => (c.id === courtId ? { ...c, price_per_hour: Number(tempPrice) } : c)) } : v)
      );
      setEditingId(null);
    } catch {
  ...
  const handleCourtAdded = (venueId, newCourt) => {
    setLocalVenues((prev) =>
      prev.map((v) => (v.id === venueId ? { ...v, courts: [...v.courts, newCourt] } : v))
    );
  };
      ...
      {localVenues.map((venue) => (
        <div key={venue.id} className="owner-venue-block">
          <div className="owner-venue-block-header">
            <span className="owner-venue-row-name" style={{ fontWeight: 700 }}>{venue.name}</span>
            ...
            <div key={court.id} className="owner-court-row">
              ...
              {editingId === court.id ? (
                <div className="owner-court-edit-group">
                  ...
                  <button onClick={() => handleSave(venue.id, court.id)} disabled={saving} className="owner-btn-save">حفظ</button>
                  <button onClick={() => setEditingId(null)} className="owner-btn-cancel">إلغاء</button>
                </div>
              ) : (
                <div className="owner-court-edit-group">
                  <span className="owner-court-price">{court.price_per_hour} ج.م/ساعة</span>
                  <button onClick={() => startEdit(court)} className="owner-court-edit-btn">تعديل</button>
                </div>
              )}
            </div>
          ))}
